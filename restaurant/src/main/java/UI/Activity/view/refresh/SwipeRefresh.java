package UI.Activity.view.refresh;
...
public class SwipeRefresh extends ViewGroup {
    ...
    protected boolean mRefreshingForBottom;
    ...
    protected boolean mUsingCustomStart;

    protected Mode mMode = Mode.DISABLED;

    protected boolean mSuperInterceptEvent = false;

    protected boolean mChildInterceptEvent = false;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (mChildInterceptEvent) {
            return false;
        }
        if (!canExecuteDownRefresh()) {
            return false;
        }
        ensureTarget();
        ...
        if (!isEnabled() || mReturningToStart || canChildScrollUp() || mRefreshing || mRefreshingForBottom) {
    ...
    protected float getMotionEventY(MotionEvent ev, int activePointerId) {
    ...
    protected boolean canExecuteDownRefresh() {
    ...
    protected boolean canExecuteUpRefresh() {
